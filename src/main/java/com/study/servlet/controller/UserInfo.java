...
public class UserInfo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// content-type에 따라 text로 인식할건지 html로 인식할건지 구분함
//		response.addHeader("Content-Type", "text/html;charset=UTF-8");
//		response.addHeader("test", "test data");	// key, value
...
//		String json = bufferedReader.lines().collect(Collectors.joining());
		...
		/**
		 *	1. 
		 *	<form action="요청메세지" method="post">	=> <form action="http://localhost:8080/Servlet_Study_nao/user" method="post">
		 *		<input name="key" value="value">
		 *		<buttom type="submit">요청</button>
		 *	</form>
		 */
	}
